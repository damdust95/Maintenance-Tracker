...
app.post("/api/timers/:filler/stop", function (req, res) {
  try {
    ...
    var now = Date.now();
    var startTime = (timer && timer.start_time) ? timer.start_time : now;
    
    // Sanity: if start_time is in the future or more than 24h ago, cap it
...
app.get("/api/technicians", function (req, res) {
  try {
    var rows = db.prepare("SELECT * FROM technicians ORDER BY shift, name").all();
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/technicians", function (req, res) {
  try {
    ...
    db.prepare("INSERT INTO technicians (id, name, shift) VALUES (?, ?, ?)").run(id, name, shift);
...
app.put("/api/technicians/:id", function (req, res) {
  try {
    var shift = (req.body.shift || "1st").trim();
    db.prepare("UPDATE technicians SET shift = ? WHERE id = ?").run(shift, req.params.id);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
