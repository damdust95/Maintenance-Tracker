...
app.post("/api/catalog", async function (req, res) {
  try {
    ...
    if (p.partNumber && p.partNumber.trim()) {
      var existing = (await pool.query("SELECT id,name FROM parts_catalog WHERE part_number=$1", [p.partNumber.trim()])).rows[0];
      if (existing) {
        return res.status(409).json({ error: "Part number already exists", existingId: existing.id, existingName: existing.name, duplicate: true });
      }
    }
    var id = p.id || uid();
    await pool.query(
...
app.post("/api/catalog/:id/equipment", async function (req, res) {
  try {
    var equipment = (req.body.equipment || "").trim();
    if (!equipment) return res.status(400).json({ error: "Equipment name required" });
    await pool.query("INSERT INTO part_equipment (part_id,equipment) VALUES ($1,$2) ON CONFLICT DO NOTHING", [req.params.id, equipment]);
    await pool.query("UPDATE parts_catalog SET usage_count=usage_count+1, last_used=CURRENT_DATE::TEXT WHERE id=$1", [req.params.id]);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/technicians/:id/break", async function (req, res) {
  try {
    var status = req.body.status || null; // null = working, 'lunch', 'break'
    await pool.query("UPDATE technicians SET break_status=$1 WHERE id=$2", [status, req.params.id]);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
initDB().then(function () {
  ...
  console.log("Starting server anyway for migration/debug...");
  app.listen(PORT, function () {
    console.log("Maintenance Tracker running on port " + PORT + " (database may not be ready)");
  });
});
