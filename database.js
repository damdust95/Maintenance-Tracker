...
function initDB() {
  ...
  // Add part_type column to event_parts if not exists
  try { db.exec("ALTER TABLE event_parts ADD COLUMN part_type TEXT"); } catch(e) {}
  // Add work_order to events if not exists
  try { db.exec("ALTER TABLE events ADD COLUMN work_order TEXT"); } catch(e) {}
  // Add time_edit_reason to events if not exists
  try { db.exec("ALTER TABLE events ADD COLUMN time_edit_reason TEXT"); } catch(e) {}
  // Add shift column to technicians if not exists
  try { db.exec("ALTER TABLE technicians ADD COLUMN shift TEXT DEFAULT '1st'"); } catch(e) {}
  ...
  var techCount = db.prepare("SELECT COUNT(*) as c FROM technicians").get().c;
  if (techCount === 0) {
    var techStmt = db.prepare("INSERT OR IGNORE INTO technicians (id, name, shift) VALUES (?, ?, ?)");
    var techs = [
    ...
    techs.forEach(function(t) {
      techStmt.run(Date.now().toString(36) + Math.random().toString(36).slice(2,7), t[0], t[1]);
    });
  }
